...
`forge-tool-pack-scope` is centered on cli tools work in Python. The technical target is: Package a Python local lab for pack analysis with transition tables, invalid-transition tests, and documented operating limits.
...
The score is deterministic. The current threshold is 175. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 6.
